...
-- 실습 4-8
select sum(sale), avg(sale) from sales where year=2019 and month=2 order by sale > 50000;
...
select * from member as a join department b on a.dep = b.depNo;
...
select a.uid, year, sum(sale) as 합계 from sales as a
join member as b on a.uid = b.uid
